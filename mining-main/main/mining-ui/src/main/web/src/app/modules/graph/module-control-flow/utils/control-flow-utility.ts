...
import { DEFAULT_NODE_SHAPE } from '../../utils/graph-global-styles';
import { ControlFlowEdge, ControlFlowGraph, ControlFlowNode } from '@innowake/mining-api-angular-client';
...
export class ControlFlowUtility {
   ...
   */
  // @yjs:keep
  getGraphInfo(graph: ControlFlowGraph, moduleName: string): ControlFlowGraphInfo {
    const graphInfo: ControlFlowGraphInfo = new ControlFlowGraphInfo();
    const edgeMap: { [fromTo: string]: ControlFlowEdge } = {};
    const nodeMap: { [id: string]: ControlFlowNodeDetails } = {};
    ...
    graph.nodes.forEach((node: ControlFlowNode) => {
      ...
      const cfgNode: ControlFlowNodeDetails = new ControlFlowNodeDetails(node, moduleName, null);
    ...
    const groupMap: { [id: string]: ControlFlowNodeDetails } = {};
    /* Build groups */
    graph.nodes.forEach((node: ControlFlowNode) => {
      if (node.superTypes != null && Array.from(node.superTypes).includes('CfgCollapsibleNode')) {
        /* create new group */
        const cfgGroup = new ControlFlowNodeDetails(node, moduleName, this.getFirstParentGroup(node, groupMap, nodeMap)?.recordId, true);
   ...
   */
  // @yjs:keep
  getFirstOutgoingNode(nodeId: string, graph: ControlFlowGraph, nodeMap: { [id: string]: ControlFlowNodeDetails }, visitedNodes: Set<string>): string {
  ...
  getFirstParentGroup(
    ...
    groupMap: { [id: string]: ControlFlowNodeDetails },
    nodeMap: { [id: string]: ControlFlowNodeDetails } = {}
   ...
   */
  processCfgNodeEdge(controlFlowEdge: ControlFlowEdge, edgeMap: { [fromTo: string]: ControlFlowEdge }, graphInfo: ControlFlowGraphInfo): void {
    const fromId = controlFlowEdge.fromId;
    const toId = controlFlowEdge.toId;
