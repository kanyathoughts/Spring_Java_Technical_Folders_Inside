...
/**
 * Enables profiling of classes implementing the {@link innowake.mining.server.datalineage.operations.DataLineageTracer} interface.
 * <p>
...
public class DataLineageTracerProfilingAspect {

	private static final Logger LOG = LoggerFactory.getLogger(DataLineageTracerProfilingAspect.class);
	...
	public DataLineageTracerProfilingAspect() {
	...
	@Pointcut("execution(* innowake.mining.server.datalineage.operations.DataLineageTracer+.*(..))")
	public void anyMethodOnDataLineageTracer() {
		/* pointcut declaration */
	}

	@Around("anyMethodOnDataLineageTracer()")
	public Object profileTracerAdvice(final ProceedingJoinPoint pjp) throws Throwable {
		return profileTracer(pjp);
	}

	private Object profileTracer(final ProceedingJoinPoint pjp) throws Throwable {
		final String tracerName = ClassUtils.getUserClass(pjp.getThis()).getSimpleName();
		final String profilingId = PROFILING_CATEGORY_PREFIX + "." + tracerName;
		...
		LOG.trace(() -> "profiling " + methodName + " on " + tracerName + " " + moduleName);
