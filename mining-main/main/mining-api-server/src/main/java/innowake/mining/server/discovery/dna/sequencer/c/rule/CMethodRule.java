 ...
 */
public class CMethodRule implements SequencerRule<AstModel> {

	@Override
	public DnaSequencer getId() {
		return DnaSequencer.C_METHOD_RULE;
	}
	...
	public void apply(final DNACollector<AstModel> collector) throws DiscoveryException {
		...
		if (node.isPresent()) {
			final Visitor<AstNode> visitor = new CMethodRuleVisitor(collector);
			visitor.visit(node.get().getAstNode());
		}

	}

	private static class CMethodRuleVisitor extends CDNAVisitor {
		...
		static {
			final String[][] stringMapArray = new String[][] {
					{
							"CDoWhileStatement", "control"
					}, {
							"CFileAccessStatement", "file_access"
					}, {
							"CForStatement", "control"
					}, {
							"CFunctionDefinition", "function_definition"
					}, {
							"CFunctionReference", "function_reference"
					}, {
							"CIfBranchExpression", "conditional"
					}, {
							"CLibraryFunction", "function_library"
					}, {
							"CContinueStatement", "continue"
					}, {
							"CReturnStatement", "return"
					}, {
							"CSwitchBranchExpression", "conditional"
					}, {
							"CDeclarationSpecifiers", "declare"
					}, {
							"CWhileStatement", "control"
					}, {
							"ExecSql", "ExecSql"
					}
			};
		...
		private CMethodRuleVisitor(final DNACollector<AstModel> collector) {
			super(collector);
		}
