...
/**
 * Task that executes dependency resolution for a dependency definition.
 */
class DependencyResolutionTask extends Task<Serializable> {
	...
	/**
	 * Creates a new DependencyResolutionTask.
	 * 
	 ...
	 */
	public DependencyResolutionTask(final JobMonitor jobMonitor, final EntityId projectId, final int iteration, final ModuleParameters moduleParameters, final EntityId moduleId) {
	...
	protected Result<Serializable> run(final ProgressMonitor progressMonitor) {
		try (final DiscoveryContext context = discoveryContextProvider.createContext(projectId, getJobId(), iteration, moduleParameters, progressMonitor,
			...
			final List<DependencyDefinitionPojo> unresolvedDependencyDefinitions = orientDiscoveryPersistence.fetchUnresolvedDependencyDefinitions(moduleId);
			unresolvedDependencyDefinitions
					.forEach(unresolvedDependencyDefinition -> {
						try {
							discoveryCore.resolveDependency(context, moduleId, unresolvedDependencyDefinition);
						} catch (final Exception e) {
							final var errorMessage = "DependencyResolutionTask: Error while creating Dependency for moduleId : '" + moduleId + " Failed Dependency definition: "
									+ unresolvedDependencyDefinition;
