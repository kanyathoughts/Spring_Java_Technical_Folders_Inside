...
/**
 * Task that executes anchoring of the unanchored ContributorResult
 */
class AnchorTask extends Task<Serializable> {
	 ...
	 */
	public AnchorTask(final JobMonitor jobMonitor, final EntityId projectId, final int iteration,
			final ModuleParameters moduleParameters, final ContributorResult contributorResult) {
	...
	protected Result<Serializable> run(final ProgressMonitor progressMonitor) {
		try (final DiscoveryContext context = discoveryContextProvider.createContext(projectId, getJobId(), iteration, moduleParameters, progressMonitor,
			...
			discoveryCore.anchorAndImportResult(context, contributorResult);
			return new Result<>(Boolean.TRUE);
		} catch (final Exception e) {
			final var errorMessage = "Error occurred while executing anchor task with module filter: " + contributorResult.getModuleFilter();
