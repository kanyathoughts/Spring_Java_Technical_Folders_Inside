 ...
 */
public class ReachabilityBottomUpTask extends Task<ReachabilityTaskResult> {

	private static final Logger LOG = LoggerFactory.getLogger(ReachabilityBottomUpTask.class);
	@Autowired
	 ...
	 */
	public ReachabilityBottomUpTask(final ProgressMonitor progressMonitor, final String jobId,
	...
	protected Result<ReachabilityTaskResult> run(final ProgressMonitor progressMonitor) {
		...
		try {
			...
			functionalBlockUids.addAll(functionalBlockGenerationService.generate(ReachabilityBottomUpGeneration.class, context, moduleId).stream()
					.filter(op -> ! op.getKey().equals(FunctionalBlockGenerationResult.Operation.DELETE)).map(Pair::getValue)
					.toList());
		} catch (final Exception ex) {
			LOG.error("Reachability Bottom-Up Task for module " + moduleId + " has failed", ex);
