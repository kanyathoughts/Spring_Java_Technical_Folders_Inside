...
public class DataPointQueryResult {
	...
	public int getSize() {
		final Integer size = getDataPoint("numberOfElements").getValue();
		return Optional.ofNullable(size).orElse(0);
	}

	public int getTotalPages() {
		final Integer totalPages = getDataPoint("totalPages").getValue();
		return Optional.ofNullable(totalPages).orElse(0);
	}
}
