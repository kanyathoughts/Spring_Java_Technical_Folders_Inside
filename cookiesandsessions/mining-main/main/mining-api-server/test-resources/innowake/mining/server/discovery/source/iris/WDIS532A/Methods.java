...
public class Methods {
	...
	public CallableStatement getCallable1(final String call) {
		...
		try {
			 ...
			 */
			final CallableStatement lokalSP1 = connection.prepareCall("CALL 1_DSN8.DSN8ED2(?,?,?,?,?)");
			if (lokalSP1.execute()) {
	...
	public void getCallable2() {
		try {
			 ...
			 */
			final CallableStatement lokalSP1 = connection.prepareCall("CALL 2a_DSN8.DSN8ED2(?,?,?,?,?)");
			if (lokalSP1.execute()) {

				/*
				 * Entry 4 in expected file
				 */
				final CallableStatement lokalSP2 = connection.prepareCall("CALL 2b_DSN8.DSN8ED2(?,?,?,?,?)");
				lokalSP2.cancel();
			}
		} catch (SQLException e) {
			throw new IllegalStateException(e);
	...
	public CallableStatement getCallable3() {
		 ...
		 */
		return getCallable1("CALL 3_DSN8.DSN8ED2(?,?,?,?,?)");
	}
	...
	public CallableStatement getCallable9(final String schema, final String name) {
		try {
			/*
			 * Entry 15 in expected file
			 */
			return connection.prepareCall("CALL " + schema + "." + name + "(?,?,?,?,?)");
		} catch (SQLException e) {
			throw new IllegalStateException(e);
		}
	}
