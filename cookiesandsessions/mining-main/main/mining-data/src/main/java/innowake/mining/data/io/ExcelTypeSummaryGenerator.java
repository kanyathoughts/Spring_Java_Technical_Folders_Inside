...
/**
 * Creates the Type Summary sheet for the Effort Summary Excel workbook.
 */
@Service
public class ExcelTypeSummaryGenerator extends EffortSummaryGenerator {
	...
	void createSheet(final SXSSFWorkbook workbook, final EntityId projectId) throws IOException {
		final Sheet sheetDefinition = EffortSummaryWorkbookDefinition.SHEETS.get(EffortSummaryWorkbookDefinition.SHEET_TYPE_SUMMARY);
		sheetGenerator = new SheetGenerator(workbook, sheetDefinition, maxRowsPerSheet);
		process(projectId, EffortSummaryType.TYPE);
	}
