...
package class BoltDecisionProjectSheetReader extends SheetReader {
	/** 
	 */
	...
	def <T> List<T> readObjects(List<T> result, Class<T> type, Object parentObject,
		...
		for (TableLayout tableLayout : tableLayouts) {
			...
			// tableInstances);
			// }
			if (tableLayout.getName().equals("decisionRuleDefinitionLayout")) {
			...
			} else if (tableLayout.getName().equals("decisionRuleConditionLayout")) {
			...
			} else if (tableLayout.getName().equals("decisionRuleConditionColumnLayout")) {
				val List<ConditionColumn> tableInstances = this.createInstances(ConditionColumn, tableRows);
				this.mapper.setProperty(instance, "conditionColumns", tableInstances);

				val ArrayList<ArrayList<NameValuePair>> conditionDataList = new ArrayList<ArrayList<NameValuePair>>();
				for (ArrayList<NameValuePair> columnDataList : tableRows) {
					if (columnDataList.size > 2) {
						val ArrayList<NameValuePair> columnData = new ArrayList<NameValuePair>( columnDataList.subList(2, columnDataList.size));

						val ArrayList<NameValuePair> conditionData = new ArrayList<NameValuePair>();
						for (NameValuePair data : columnData) {
							val ArrayList<NameValuePair> rowData = new ArrayList<NameValuePair>();
							rowData.add(data);

							val ArrayList<ArrayList<NameValuePair>> dataRows = new ArrayList<ArrayList<NameValuePair>>();
							dataRows.add(rowData);

							val List<NameValuePair> dataInstances = this.createInstances(NameValuePair, dataRows);
							conditionData.addAll(dataInstances);
						}
						conditionDataList.add(conditionData);
					}
				}
				this.mapper.setProperty(instance, "conditionData", conditionDataList);
			} else if (tableLayout.getName().equals("decisionRuleActionColumnLayout")) {
				val List<ActionColumn> tableInstances = this.createInstances(ActionColumn, tableRows);
				this.mapper.setProperty(instance, "actionColumns", tableInstances);
			
				val ArrayList<ArrayList<NameValuePair>> actionDataList = new ArrayList<ArrayList<NameValuePair>>();
				for (ArrayList<NameValuePair> columnDataList : tableRows) {
					if (columnDataList.size > 2) {
						val ArrayList<NameValuePair> columnData = new ArrayList<NameValuePair>( columnDataList.subList(2, columnDataList.size));

						val ArrayList<NameValuePair> actionData = new ArrayList<NameValuePair>();
						for (NameValuePair data : columnData) {
							val ArrayList<NameValuePair> rowData = new ArrayList<NameValuePair>();
							rowData.add(data);
							...
							val List<NameValuePair> dataInstances = this.createInstances(NameValuePair, dataRows);
							actionData.addAll(dataInstances);
						}
						actionDataList.add(actionData);
					}
				}
				this.mapper.setProperty(instance, "actionData", actionDataList);
			}
			System.out.print("Completed - " + tableLayout.getName());
		}
		if (instance != null) {
			result.add(instance);
		}
