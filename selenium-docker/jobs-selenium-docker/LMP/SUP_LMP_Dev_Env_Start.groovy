...
/**
 * Job script to build and deploy the License Management Portal on the selected server.
 *
 * @param useDeloitteLdap If true, the user credentials are authenticated with the Deloitte LDAP.
 * @param productiveEnvironment The Jenkins node the test will run on
 * @param httpsConfiguration If true, https configuration should be loaded.
...
node('USLinux1') {
    timestamps {
        ...
        httpsConfiguration = Boolean.parseBoolean(httpsConfiguration)
        buildName "#${env.BUILD_ID} - LMP Dev"
        buildDescription "useDeloitteLdap=${useDeloitteLdap}, productiveEnvironment=${productiveEnvironment}, branchName=${branchName}, httpsConfiguration=${httpsConfiguration}"
        ...
        // order is important
        def ctrNames = [
                gitCtr      : 'lmp_gitclone_dev',
                dbCtr       : 'lmp_db_dev',
                waitForDBCtr: 'lmp_waitfordb_dev',
                prepareWSCtr: 'lmp_preparews_dev',
                npmCtr      : 'lmp_providenpm_dev',
                webImageCtr : 'lmp_web_dev',
                uiImageCtr  : 'lmp_ui_dev'
        ]
        def networkName = 'lmp_network_dev'
        ...
        stage('initialize') {
            ...
            //initialize database
            withCredentials([usernamePassword(credentialsId: 'LMP_MySQL_User_Dev', passwordVariable: 'mySQLPw', usernameVariable: 'mySQLUser')]) {
                docker.image('mysql:5.7').run("--name ${ctrNames["dbCtr"]} --network ${networkName} -e MYSQL_DATABASE=${mySQLDBTable} -e MYSQL_ROOT_PASSWORD=${mySQLPw} -e MYSQL_PASSWORD=${mySQLPw}")
            }
            docker.image('mysql:5.7').inside("--name ${ctrNames["waitForDBCtr"]} --network ${networkName} --link ${ctrNames["dbCtr"]}:db") {
                //wait until mysql service is up. Since link was used, here dbCtr is the same as db
            ...
            if (httpsConfiguration) {
                ...
                dir('LMPUI/nginx_conf') {
                    sh "sed -i 's,listen 443 ssl;,listen       80;,g' custom.conf"
                    sh "sed -i 's,ssl_certificate /etc/nginx/certs/ussltc8434v_consulting_sltc_com_bundle.crt;,,g' custom.conf"
                    sh "sed -i 's,ssl_certificate_key /etc/nginx/certs/ussltc8434v_consulting_sltc_com.key;,,g' custom.conf"
                }
            }
        }

        docker.image(dockerUtils.pullJenkinsEnvironmentImage('java8')).inside("--name ${ctrNames["prepareWSCtr"]} --network ${networkName} --link ${ctrNames["dbCtr"]} -v /data/mxJars:/data/mxJars:ro -v jenkins-m2-repo:/var/m2_repo") {
            stage('customize LMP') {
                dir('LMPModifiers') {
                ...
                dir('LMPService/src/main/resources') {
                    ...
                    yamlContent.url = "jdbc:mysql://${ctrNames["dbCtr"]}:3306/${mySQLDBTable}"
                    withCredentials([usernamePassword(credentialsId: 'LMP_MySQL_User_Dev', passwordVariable: 'mySQLPw', usernameVariable: 'mySQLUser')]) {
                        yamlContent.dbUserName = mySQLUser
                        yamlContent.dbPassword = mySQLPw
                    }
                    yamlContent.isLdapActive = useDeloitteLdap
                    sh "rm ${filename}"
                    writeYaml file: filename, data: yamlContent
                }
                dir('LMPUI/public/css') {
                    sh "sed -i 's,9816f4,9ACD66,g' vendors.bundle.css"
                }
                ...
                dir("${workDir}/LMPWeb/target") {
                    dir(metaInfFolderPath) {
                        ...
                        sh "sed -i 's,docker-mysql:3306/licensing,${ctrNames["dbCtr"]}:3306/${mySQLDBTable},g' persistence.xml"
                        withCredentials([usernamePassword(credentialsId: 'LMP_MySQL_User_Dev', passwordVariable: 'mySQLPw', usernameVariable: 'mySQLUser')]) {
        ...
        stage('build LMP') {
            dir('LMPUI/public') {
                if (httpsConfiguration) {
                    ...
                    sh "sed -i 's,http://localhost:8080,http://${hostname}:${webPort}/LMPWeb,g' config.json"
                }
            }
            dir('LMPUI') {
                def user = miscUtils.getUserID()
                def group = miscUtils.getGroupID()
                docker.image('node:8.9.4').inside("--name ${ctrNames["npmCtr"]} --network ${networkName} --link ${ctrNames["dbCtr"]} -u=0 -v ${workDir}/LMPUI/package.json:/package.json -v ${workDir}/LMPUI/package-lock.json:/package-lock.json -v jenkins-m2-repo:/var/m2_repo:rw") {
                    sh 'node -v'
                    sh 'npm -v'
        ...
        stage('start LMP') {
            docker.image('tomcat:9.0.37-jdk8-openjdk').withRun("--name ${ctrNames["webImageCtr"]} --restart always --network ${networkName} --link ${ctrNames["dbCtr"]} -v ${workDir}/LMPWeb/target/LMPWeb.war:/usr/local/tomcat/webapps/LMPWeb.war -v ${workDir}/LMPWeb/config/server.xml:/usr/local/tomcat/conf/server.xml -v ${workDir}/LMPWeb/config/web.xml:/usr/local/tomcat/conf/web.xml -v ${keysFolder}:/usr/local/keystore -p ${webPort}:8443") {
                docker.image('nginx:1.17.1-alpine').withRun("--name ${ctrNames["uiImageCtr"]} --restart always --network ${networkName} --link ${ctrNames["dbCtr"]} --link ${ctrNames["webImageCtr"]} -v ${workDir}/LMPUI/dist:/usr/share/nginx/html -v ${workDir}/LMPUI/nginx_conf:/etc/nginx/conf.d -v ${keysFolder}:/etc/nginx/certs -p ${uiPort}:443 -v /etc/nginx") {
                    sleep(2)
                    try {
