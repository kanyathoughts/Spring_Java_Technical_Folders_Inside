...
node('USLinux1') {
    timestamps {
        ...
        buildName "#${env.BUILD_ID} - LMP Prod"
        ...
        // order is important
        def ctrNames = [
                gitCtr      : 'lmp_gitclone_prod',
                prepareWSCtr: 'lmp_preparews_prod',
                npmCtr      : 'lmp_providenpm_prod',
                webImageCtr : 'lmp_web_prod',
                uiImageCtr  : 'lmp_ui_prod'
        ]
        def networkName = 'lmp_network_prod'
        ...
        docker.image(dockerUtils.pullJenkinsEnvironmentImage('java8')).inside("--name ${ctrNames["prepareWSCtr"]} --network ${networkName} -v /data/mxJars:/data/mxJars:ro -v jenkins-m2-repo:/var/m2_repo") {
            stage('customize LMP') {
                ...
                dir('LMPService/src/main/resources') {
                    ...
                    yamlContent.url = 'jdbc:mysql://10.25.44.217:3306/production_innowake_activation?serverTimezone=UTC'
                ...
                dir("${workDir}/LMPWeb/target") {
                    dir(metaInfFolderPath) {
                        ...
                        sh "sed -i 's,docker-mysql:3306/licensing,10.25.44.217:3306/production_innowake_activation?serverTimezone=UTC,g' persistence.xml"
                        withCredentials([usernamePassword(credentialsId: 'LMP_MySQL_User_Prod', passwordVariable: 'mySQLPw', usernameVariable: 'mySQLUser')]) {
        ...
        stage('start LMP') {
            docker.image('tomcat:9.0.37-jdk8-openjdk').run("--name ${ctrNames["webImageCtr"]} --restart always --network ${networkName} -v ${workDir}/LMPWeb/target/LMPWeb.war:/usr/local/tomcat/webapps/LMPWeb.war -v ${workDir}/LMPWeb/config/server.xml:/usr/local/tomcat/conf/server.xml -v ${workDir}/LMPWeb/config/web.xml:/usr/local/tomcat/conf/web.xml -v ${keysFolder}:/usr/local/keystore -p ${webPort}:8443")
            docker.image('nginx:1.17.1-alpine').run("--name ${ctrNames["uiImageCtr"]} --restart always --network ${networkName} --link ${ctrNames["webImageCtr"]} -v ${workDir}/LMPUI/dist:/usr/share/nginx/html -v ${workDir}/LMPUI/nginx_conf:/etc/nginx/conf.d -v ${keysFolder}:/etc/nginx/certs -p ${uiPort}:443 -v /etc/nginx")
