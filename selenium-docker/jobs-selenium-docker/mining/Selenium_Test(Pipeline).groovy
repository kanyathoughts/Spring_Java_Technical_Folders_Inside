...
node(executeEnvironmentOn) {
	...
	timestamps {
		...
		def startMiningEnvironmentResultKanya = 'SUCCESS'
		
		if (startMiningEnvironmentResultKanya == 'SUCCESS') {
			...
			testCases.each {
				...
				stage (stageName) {
					def buildResult = build job: 'Selenium_Test', propagate: false, parameters: [ 
					...
					copyArtifacts filter: 'artifact/reports/*/**', optional: true, projectName: 'Selenium_Test', selector: specific(buildResult.number.toString()), target: "${workDir}/${testCase}"	
				}
			}
		} else {
