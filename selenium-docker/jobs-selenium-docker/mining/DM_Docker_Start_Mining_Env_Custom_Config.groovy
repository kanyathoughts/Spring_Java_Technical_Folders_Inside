...
node(executeOn) {
 ...
 timestamps {
	...
	def gitDirCustomConfig = "${jobWorkspace}/gitCustomConfig"
    ...
    stage('get selected build and checkout docker files') {
		...
		docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside {
            withCredentials([string(credentialsId: 'USAppModQMUserSVC-Access-Token', variable: 'gitlabToken')]) {
				...
				gitlabProjectCustomConfig = "${gitUtils.getGitUrlQef('USAppModQMUserSVC-Access-Token', gitlabToken)}/infrastructure/mining-custom-config.git"
            }
			dir(dockerDir) {
			...
			dir(gitDirCustomConfig) {
				sh "git clone --depth 1 --branch master ${gitlabProjectCustomConfig} ${gitDirCustomConfig}"
			}
    ...
    stage('setup and run api server') {
		def startCommand = "nohup java -jar -Dserver.port=8080 -Djava.awt.headless=true -Dspring.datasource.url=jdbc:orient:remote:${serverIPToRunOn}:${OrientDBbin}/mining ./mining-api-server-dist.jar --spring.config.additional-location=./application-custom.yml --mining.cookieId=DISABLED"
        if (withKeycloak) {
        ...
        sh "chmod 777 ${gitDirCustomConfig}/application-custom.yml"
        docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).run("--name ${apiServerName} -p ${APIServer}:8080 -v ${jobWorkspace}/mining-api-server-dist.jar:/mining-api-server-dist.jar -v ${gitDirLicenses}/mining/innowake.lic:/innowake.lic -v ${gitDirCustomConfig}/application-custom.yml:/application-custom.yml", "bash -c \"${startCommand}\"")
        sleep 120
