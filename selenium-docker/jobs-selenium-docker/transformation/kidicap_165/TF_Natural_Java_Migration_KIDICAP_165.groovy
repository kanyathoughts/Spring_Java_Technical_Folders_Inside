		...
		try {
			...
			docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside('-v jenkins-m2-repo:/var/m2_repo:rw') {
				...
				stage('setup-test-workspace') {
					...
					antProperties = "-DworkDir=${workDir} -DjvmDir=java -DjavaVersion=${miscUtils.getInstalledJavaVersion()} -DmxBuildVersion=${mxBuildVersion} -DwithCodeCoverage=${withCodeCoverage} -DtransformCopycodes=${transformCopycodes}"
				...
				stage('init-nat2java') {
                	withAnt(installation: 'Default') {
                		...
                		sh "ant ${antFlags} -buildfile ${antFile_nat2Java} ${antProperties} -DtransformCopycodes=${transformCopycodes} -DmxJarsDir=${fakeDataMxJarsDir} init-nat2java"
                	}
				}
			...
			stage('finalize') {				
				docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside('-v jenkins-m2-repo:/var/m2_repo:rw') {
					def suffix = "_transformCopycodes=${transformCopycodes}"
					archiveArtifacts allowEmptyArchive: true, artifacts: 'log/**/*, coverage/**/*,**/*.exec'
					sh "mv KIDICAP_LIBS/kidicap.jar KIDICAP_LIBS/kidicap${suffix}.jar"
					spUtils.uploadJobArtifact(mxBuildVersion, "KIDICAP_LIBS/kidicap${suffix}.jar")
					sh "mv KIDICAP_LIBS/kidicap-src.jar KIDICAP_LIBS/kidicap-src${suffix}.jar"
					spUtils.uploadJobArtifact(mxBuildVersion, "KIDICAP_LIBS/kidicap-src${suffix}.jar")					
					sh "mv KIDICAP_LIBS/bflex-agtv.jar KIDICAP_LIBS/bflex-agtv${suffix}.jar"
					spUtils.uploadJobArtifact(mxBuildVersion, "KIDICAP_LIBS/bflex-agtv${suffix}.jar")
					sh "mv KIDICAP_LIBS/bflex-ofdan.jar KIDICAP_LIBS/bflex-ofdan${suffix}.jar"
					spUtils.uploadJobArtifact(mxBuildVersion, "KIDICAP_LIBS/bflex-ofdan${suffix}.jar")
					sh "mv result-comparison.zip result-comparison${suffix}.zip"
					spUtils.uploadJobArtifact(mxBuildVersion, "result-comparison${suffix}.zip")
				}
			}
		}		
	}	
