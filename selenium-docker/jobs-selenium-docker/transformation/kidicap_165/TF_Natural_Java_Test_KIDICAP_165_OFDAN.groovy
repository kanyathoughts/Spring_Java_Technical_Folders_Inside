...
nodeTF('Docker-host && Region-EU') {
	...
	timestamps {
		...
		def dbPort = '1521'
		def dbSchema = 'KIDICAP_R165_OFDAN'
		...
		def dockerImage = dockerUtils.pullImage('gip-165-ofdan-mx16', mxVersion)
		def workDir
		def antFlags = ''
		def antProperties
		...
		buildDescription "kidicapBuild=${kidicapBuild} parallelMode=${parallelMode} reduceDbVolume=${reduceDbVolume} mergeLess=${mergeLess} withCodeCoverage=${withCodeCoverage} javaVersion=${javaVersion} testProjectBranch=${testProjectBranch}"
		...
		try {
			...
			docker.image(dockerImage).withRun {
				...
				stage('reload-UDF-jar') {
					buildProperties = miscUtils.readPropertyFile('build.properties', ['workDir': workDir, 'bflexVariant': 'ofdan', 'mxBuildVersion': mxBuildVersion, 'dbPort': dbPort])
					def hostname = 'localhost'
					def dbConnectString
					withCredentials([usernamePassword(credentialsId: 'gip165ofdan', passwordVariable: 'pass', usernameVariable: 'user')]) {
                ...
                docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside("--network=container:${container.id} -v jenkins-m2-repo:/var/m2_repo:rw") {
					...
					stage('initialisation') {
		                ...
		                spUtils.downloadJobArtifact('TF_Natural_Java_Migration_KIDICAP_165', kidicapBuild, 'kidicap_transformCopycodes=false.jar', 'KIDICAP_LIBS')
						sh 'mv KIDICAP_LIBS/kidicap_transformCopycodes=false.jar KIDICAP_LIBS/kidicap.jar'
						spUtils.downloadJobArtifact('TF_Natural_Java_Migration_KIDICAP_165', kidicapBuild, 'bflex-ofdan_transformCopycodes=false.jar', 'KIDICAP_LIBS')
						sh 'mv KIDICAP_LIBS/bflex-ofdan_transformCopycodes=false.jar KIDICAP_LIBS/bflex-ofdan.jar'
						antProperties = "-DworkDir=${workDir} -DjvmDir=java -DjavaVersion=${miscUtils.getInstalledJavaVersion()} -DmxBuildVersion=${mxBuildVersion} -DdbPort=${dbPort} -DparallelMode=${parallelMode} -DmergeLess=${mergeLess} -DwithCodeCoverage=${withCodeCoverage}"
						...
						productionAntFile = "${workDir}/run-production-ofdan.xml"
						prepareDbAntFile  = "${workDir}/prepare-DB.xml"
						buildProperties = miscUtils.readPropertyFile("${workDir}/build.properties", ['workDir': workDir, 'bflexVariant': 'ofdan', 'mxBuildVersion': mxBuildVersion, 'dbPort': dbPort])
					    ...
					    if (reduceDbVolume) {
							...
							def dbPropsReduce = "${antPropertiesDbCommon} -DdbUser=${buildProperties.ofdanDbSchema} -DdbPassword=${buildProperties.ofdanDbPassword} -DreduceDbExecutionDay=${buildProperties.reduceDbExecutionDayOfdan} -DreduceDbKey=${buildProperties.reduceDbKeyOfdan}"
