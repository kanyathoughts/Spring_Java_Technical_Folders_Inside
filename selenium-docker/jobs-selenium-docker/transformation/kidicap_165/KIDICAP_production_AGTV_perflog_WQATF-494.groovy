...
nodeTF(executeOn) {
	...
	timestamps {
		...
		def testProjectBranch = (mxVersion == '99.9') ? 'master' : mxVersion
		...
		buildDescription "withCodeCoverage=${withCodeCoverage} parallelMode=${parallelMode} reduceDbVolume=${reduceDbVolume} paging=${paging} mergeLess=${mergeLess} javaVersion=${javaVersion} parallelThreads=${parallelThreads} performanceProfiling=${performanceProfiling} executeOn=${executeOn}"
		echo "mxBuildVersion: ${mxBuildVersion}, mxVersion: ${mxVersion}, withCodeCoverage: ${withCodeCoverage}, javaVersion: ${javaVersion} parallelThreads=${parallelThreads} performanceProfiling=${performanceProfiling} executeOn=${executeOn}"
			...
			if (updateDbSchema) {
				...
				docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside('-v jenkins-m2-repo:/var/m2_repo:rw') {
					...
					stage('setup-schema-generation') {
						...
						antProperties = "-DworkDir=${workDir} -DjvmDir=java -DjavaVersion=${miscUtils.getInstalledJavaVersion()} -DmxBuildVersion=${mxBuildVersion} -DdbPort=${dbPort} -DparallelMode=${parallelMode} -Dpaging=${paging} -DmergeLess=${mergeLess} -DwithCodeCoverage=${withCodeCoverage} -DdbSchema=${dbSchema}"			        		
			...
			docker.image(dockerImage).withRun {
				...
				docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside("--network=container:${container.id} -v jenkins-m2-repo:/var/m2_repo:rw") {
					...
					stage('initialization') {						
						...
						antProperties = "-DworkDir=${workDir} -DjvmDir=java -DjavaVersion=${miscUtils.getInstalledJavaVersion()} -DmxBuildVersion=${mxBuildVersion} -DdbPort=${dbPort} -DparallelMode=${parallelMode} -DparallelThreads=${parallelThreads} -Dpaging=${paging} -DmergeLess=${mergeLess} -DwithCodeCoverage=${withCodeCoverage} -Dinnowake.lib.core.profile.ProfilingEnabled=${performanceProfiling}"
