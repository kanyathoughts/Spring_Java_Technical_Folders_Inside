...
/**
 * Transforms the Cobol sources of the license validation check to C#, using a headless Eclipse.
 * 
    ...
    nodeTF('Docker-host') {        
        ...
        try {
            ...
            docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside('-v jenkins-m2-repo:/var/m2_repo:rw') {
                ...
                stage('cobol2csharp') {
                    miscUtils.prepareAndRunMigration('cobol2csharp', migDir, javaVersion, withCodeCoverage)
                    archiveArtifacts 'migration/eclipseWorkspace/Test-Project/src-cobol-csharp/**/*'
                }
            }
        } catch (ex) {
