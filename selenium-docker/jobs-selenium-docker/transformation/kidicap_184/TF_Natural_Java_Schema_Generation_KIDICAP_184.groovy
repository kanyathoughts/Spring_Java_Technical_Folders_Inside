...
/**
 * Create MySQL schemas for KIDICAP_184 production jobs.
 * 
...
nodeTF('OS-Linux && Region-EU') {
	...
	timestamps {
		...
		buildDescription "parallelMode=${parallelMode} withCodeCoverage=${withCodeCoverage} javaVersion=${javaVersion} testProjectBranch=${testProjectBranch}"
        ...
        docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside('-v jenkins-m2-repo:/var/m2_repo:rw') {
            ...
            stage(dbSchema) {
                //init
                def antProperties = "-DworkDir=${workDir} -DjvmDir=java -DjavaVersion=${miscUtils.getInstalledJavaVersion()} -DmxBuildVersion=${mxBuildVersion} -DdbPort=${dbPort} -DparallelMode=${parallelMode} -DwithCodeCoverage=${withCodeCoverage} -DdbSchema=${dbSchema}"			        		
