...
nodeTF('Docker-host && Region-EU') {
	...
	timestamps {
		...
		def dbPort = '1521'
		def dbSchema = 'KIDICAP_R154_OFDAN'
		def dockerImage = dockerUtils.pullImage('gip-154-ofdan-mx16', mxVersion)
		def workDir
		def antFlags = ''
		def antProperties
		def buildProperties
		...
		def jarsForDownload = ['bflex-ofdan', 'kidicap']
                ...
                docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside("--network=container:${container.id} -v jenkins-m2-repo:/var/m2_repo:rw") {
					...
					productionSteps.each {
						...
						stage("productionPart${step}") {
							...
							def expectedFilesDir = "${workDir}/expected/logfiles/ofdan/${subDirForStep}"
