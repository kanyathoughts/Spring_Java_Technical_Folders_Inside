...
nodeTF('Docker-host && Region-EU') {
	...
	timestamps {
		...
		def dbPort = '1521'
		def dbSchema = 'KIDICAP_R154_OFDVE'
		def dockerImage = dockerUtils.pullImage('gip-154-ofdve-mx16', mxVersion)
		def workDir
		def antFlags = ''
		def antProperties
		def buildProperties
		...
		def jarsForDownload = ['bflex-ofdve', 'kidicap']
		
		def subDirForSteps = [
			'01': 'OFDVE_01_RZ_Plausi',
			'02': 'OFDVE_02_Plausi_Protokoll',
			'03': 'OFDVE_03_Vorinstanz',
			'04': 'OFDVE_04_Berechnung',
			'05': 'OFDVE_05_BG_Abrechnung',
			'06': 'OFDVE_06_DEUEV',
			'07': 'OFDVE_07_ZVK',
			'08': 'OFDVE_08_ErgPaket',
			'09': 'OFDVE_09_Abstimmung',
			'10': 'OFDVE_10_Ueberleitung',
			'11': 'OFDVE_11_UL_Listen',
			'18': 'OFDVE_18_Bezugsartenliste',
			'23': 'OFDVE_23_Nachinstanz'
		]
			   ...
			   container ->
                ...
                docker.image(dockerUtils.pullJenkinsEnvironmentImage(javaVersion)).inside("--network=container:${container.id} -v jenkins-m2-repo:/var/m2_repo:rw") {
					...
					stage('initialisation') {
			    		...
			    		productionAntFile = "${workDir}/run-production-ofdve.xml"
					...
					productionSteps.each {
						...
						stage("productionPart${step}") {
							...
							def expectedFilesDir = "${workDir}/expected/logfiles/ofdve/${subDirForStep}"
